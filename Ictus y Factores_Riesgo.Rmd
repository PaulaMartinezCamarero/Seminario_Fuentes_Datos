...
for(i in altas_hospitalarias_ictus$Sexo){
  if(i == "Mujer"){
    for(e in altas_hospitalarias_ictus$Edad)
    if(e< 18){
      m_0_18 <- m_0_18 + 1
    } else if(e>= 19 && e<= 35){
      m_19_35 <- m_19_35 + 1
...
altas_hospitalarias_ictus %>%
  ...
  gather_object %>%
  spread_all %>%
  #select(age= edad, sex=sexo,prov=provincia,procedencia=ambito_de_procedencia,ingreso=fecha_de_ingreso) %>%
  group_by(fields.edad,fields.sexo)%>%
  summarize(mean_age = mean(fields.edad, na.rm = TRUE))
