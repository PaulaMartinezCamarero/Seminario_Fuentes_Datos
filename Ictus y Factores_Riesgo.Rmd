...
Dejamos la tabla entera tabulada
```{r}
...
#aire_con_ictus_tab <- aire_con_ictus%>%
...
calucla_media_sustancias_por_provincia_y_meses <-aire_con_ictus_resumen %>%
...
calucla_media_sustancias_por_provincia_y_meses
...
El gráfico que se muestra a continuación nos sirve para ver si cuando la concentración de NO es mayor hay un mayor número de ictus.

Grafico de dispersión del NO:
```{r}
grafica_dispersion_num_ictus_prov_NO <- calucla_media_sustancias_por_provincia_y_meses%>%
  ggplot(aes(x=avg_NO, y=num_ictus))+
  geom_point(aes(colour = factor(provincia)))+
  geom_smooth(method = "lm", aes(colour = factor(provincia)))
grafica_dispersion_num_ictus_prov_NO
...
Grafico de dispersión del 03:
```{r}
grafica_dispersion_num_ictus_prov_O3 <- calucla_media_sustancias_por_provincia_y_meses%>%
  ggplot(aes(x=avg_O3, y=num_ictus))+
  geom_point(aes(colour = factor(provincia)))+
  geom_smooth(method = "lm", aes(colour = factor(provincia)))
grafica_dispersion_num_ictus_prov_O3
```

La mortalidad cardiovascular y también las hospitalización aumentan en un día determinado entre un 0,5% y un 1% por cada aumento de 10 mg/m3 de PM2.5.
...
```{r}
grafica_dispersion_num_ictus_prov_PM25 <- calucla_media_sustancias_por_provincia_y_meses%>%
  ggplot(aes(x=avg_PM25, y=num_ictus))+
  geom_point(aes(colour = factor(provincia)))+
  geom_smooth(method = "lm", aes(colour = factor(provincia)))
grafica_dispersion_num_ictus_prov_PM25
```


Añado en la tabla anterior una columna en la que aparece el número de ictus en cada provincia, para así poder hacer las gráficas
```{r}
ictus_por_provincia <- data.frame(provincia=c("Avila","Burgos","León","Palencia","Salamanca","Segovia","Soria","Valladolid","Zamora"),
                                  numero_ictus=c(1178,2788,3513,1064,3743,1142,601,4575,1486))
ictus_por_provincia
aire_con_num_ictus <- full_join(aire_con_ictus_resumen, ictus_por_provincia, "provincia")
#aire_con_num_ictus
```

```{r}
?geom_smooth
grafica_puntos_NO <- ggplot(calucla_media_sustancias_por_provincia_y_meses, aes(x=avg_NO,y=num_ictus))+
...
```{r}
g_ictus_prov_gas_NO <- aire_con_num_ictus %>%
  distinct(provincia, .keep_all = TRUE) %>%  
  ggplot(aes(x = reorder(provincia, numero_ictus), y = numero_ictus)) + 
  geom_bar(stat = "identity", aes(fill = NO)) +
  scale_fill_gradient(low = "yellow", high = "red", space = "Lab", na.value = "grey50", guide = "colourbar") +
  labs(x = "", y = "Número ictus", fill = "Media de gas") +
  theme_classic()
...
```{r}
g_ictus_prov_gas_PM25 <- aire_con_num_ictus %>%
  distinct(provincia, .keep_all = TRUE) %>%  
  ggplot(aes(x = reorder(provincia, numero_ictus), y = numero_ictus)) + 
  geom_bar(stat = "identity", aes(fill = PM25)) +
  scale_fill_gradient(low = "yellow", high = "red", space = "Lab", na.value = "grey50", guide = "colourbar") +
  labs(x = "", y = "Número ictus", fill = "Media de gas") +
  theme_classic()

g_ictus_prov_gas_PM25
```
En esta gráfica aparecen representados los números de ictus por  provincia y la concentración de PM25, la cual aparece pintada más oscura cuando es mayor y más clara cuando es menor.

```{r}
g_ictus_prov_gas_O3 <- aire_con_num_ictus %>%
  distinct(provincia, .keep_all = TRUE) %>% 
  ggplot(aes(x = reorder(provincia, numero_ictus), y = numero_ictus)) + 
  geom_bar(stat = "identity", aes(fill = O3)) +
  scale_fill_gradient(low = "yellow", high = "red", space = "Lab", na.value = "grey50", guide = "colourbar") +
  labs(x = "", y = "Número ictus", fill = "Media de gas") +
  theme_classic()
...
He tenido que buscar información sobre como hacer que se puedan usar 3 columnas para una mismas provincia:
https://www.geeksforgeeks.org/pivot-longer-in-r/
https://r-charts.com/es/ggplot2/facetas/
https://rpubs.com/Edimer/689875

```{r}

aire_con_num_ictus_long <- aire_con_num_ictus %>%
  distinct(provincia, numero_ictus, NO, PM25, O3) %>%
  pivot_longer(cols = c(NO, PM25, O3), 
               names_to = "gas", 
               values_to = "concentracion_gas")

grafica_conc_gases_por_prov_num_ictus <- ggplot(aire_con_num_ictus_long, 
                                                 aes(x = provincia, 
                                                     y = numero_ictus, 
                                                     fill = concentracion_gas)) + 
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) + # Separar las barras
  scale_fill_gradient(low = "pink", high = "violet") + 
  labs(x = "Provincias", y = "Número de ictus",fill="Concentración de gas") +
  facet_wrap(~ gas, scales = "free_y") + 
  theme_classic() 


grafica_conc_gases_por_prov_num_ictus



```

En esta imagen, aparecen 3 gráficas, en las que se muestran el número de ictus en cada provincia y la concentración de cada gas (NO, O3 y PM25) de un color más oscuro cuando es mayor y de un color más claro cuando es menor.
Resumidamente, son las tres gráficas anteriores juntadas en una para una visualización más clara.

Como dato curioso, queremos ver si el día de la semana está relacionado con los ictus
```{r}
grafica_dias_semana_ingreso <- aire_con_num_ictus %>%
  distinct(provincia, .keep_all = TRUE) %>%  
  ggplot(mapping=aes(x = dia_de_la_semana_en_la_fecha_del_ingreso, y = numero_ictus)) + 
  geom_bar(stat = "identity", fill="purple") +
  labs(x = "Días de la semana", y="Número de ictus") +
  theme_classic()

grafica_dias_semana_ingreso

```

Ahora vamos a ver también como dato curioso el día de alta hospitalaria:
```{r}
grafica_dias_semana_alta <- aire_con_num_ictus %>%
  distinct(provincia, .keep_all = TRUE) %>%  
  ggplot(mapping=aes(x = dia_de_la_semana_en_la_fecha_del_alta, y = numero_ictus)) + 
  geom_bar(stat = "identity", fill="purple") +
  labs(x = "Días de la semana", y="Número de ictus") +
  theme_classic()

grafica_dias_semana_alta
```
...
```{r}
grafica_aire <- media_factores_aire%>%
  ggplot(mapping=aes(x=provincia, y=NO))+
...
library("pxR")
sedentarismo_px <- read.px("INPUT/data/sedentarismo.px")
sedentarismo <- as.data.frame(sedentarismo_px)
sedentarismo

```
#Importamos los datos de consumo de fruta, verduras, ensaladas y hortalizas.
```{r}
frutas_px <- read.px("INPUT/data/consumo_fruta_y_verdura.px")
frutas <- as.data.frame(frutas_px)
frutas
```

value nos da el porcentaje de gente que si consume lo indicado en tipo de alimento.
```{r}
source("INPUT/functions/ictus_por_provincia.R")
ictus_por_provincia(Provincias) #calcularemos el porcentaje que supone de la población total
source("INPUT/functions/divide.R")
source("INPUT/functions/poblacion_cyl.R")

porcentaje_ictus_provincia <- divide(ictus_por_provincia(Provincias),poblacion_cyl$Poblacion_total[1:9])*100
porcentaje_ictus_provincia
porcentaje_sin_ictus_provincia <-100-porcentaje_ictus_provincia
porcentaje_sin_ictus_provincia
```
