...
df_ictus_por_provincia <- data.frame(provincia=datos$pob.provincia,
                n_ictus=datos$num_ictus.numero_ictus)
...
```{r}
aire_con_num_ictus_long <- calcula_media_sustancias_por_provincia_y_meses %>%
  distinct(provincia, num_ictus, avg_NO, avg_PM25, avg_O3) %>%
  pivot_longer(cols = c(avg_NO, avg_PM25, avg_O3), 
               names_to = "gas", 
               values_to = "concentracion_gas")

ggplot(aire_con_num_ictus_long, 
  aes(x = provincia, y = num_ictus, fill = concentracion_gas)) + 
