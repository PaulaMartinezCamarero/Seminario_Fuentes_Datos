...
altas_hosp_ictus_fecha_bien <- altas_hosp_ictus_tab %>%
  ...
  mutate(mes= rbind(month(fields.fecha_de_ingreso)))
...
```{r}
aire_con_ictus <- full_join(altas_hosp_ictus_fecha_bien, media_factores_aire, by = c("Fecha", "provincia"))
...
aire_con_ictus_resumen <- aire_con_ictus%>%
  ...
  select(ambito,edad,sexo,dia_semana,provincia,Fecha,NO,O3,PM25)
...
```{r}
?geom_smooth
grafica_puntos_NO <- ggplot(aire_con_num_ictus, aes(x=NO,y=numero_ictus))+
